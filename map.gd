...
func _spawn_prop(world_pos: Vector2, size: WorldProp.Size, sprite_name: String, 
	variant_count: int, central_rotation: bool, has_collision: bool, destructible: bool) -> void:
	...
	if has_collision:
		...
		pathfinder.set_tile_solid(tile_key, size + 1, true)
		if destructible:
			prop.died.connect(func():
				destructible_map.erase(tile_key)
				rotatable_sprites.erase(prop.sprite)
				pathfinder.set_tile_solid(tile_key, size + 1, false)
			)
